...
public class TurningPointScene extends Scene
{
	...
	public static final Pane pane1 = new Pane();
	public static final Pane pane2 = new Pane();
	
	public TurningPointScene(Parent layout, double bufferAmount) {
		super(layout, imageXDim + (2 * bufferAmount), imageXDim + (2 * bufferAmount) + 182);
		
		System.setOut(GUI.stream);
		System.setErr(GUI.stream);
		...
		setOnMousePressed(new EventHandler<MouseEvent>() {
	        ...
	        public void handle(MouseEvent event) {
	        	if (event.getButton().equals(MouseButton.PRIMARY) && event.getY() <= imageXDim)
	        	{
